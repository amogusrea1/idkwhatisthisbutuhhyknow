...
options=("Vista rdp" "Windows8 rdp, pass is unknown:die:" "Windows 2012 R2 Datacenter with Aero Glass (Activated)" "quit")
...
do
    case $opt in
        "Vista rdp")
            ...
            echo "Windows Vista On Google Colab"
       ...
       "Windows8 rdp, pass is unknown:die:")
        ...
        echo "Windows 8 On Google Colab"
        echo Your VNC IP Address:
        curl --silent --show-error http://127.0.0.1:4040/api/tunnels | sed -nE 's/.*public_url":"tcp:..([^"]*).*/\1/p'
        echo "Note: Use Right-Click Or Ctrl+C To Copy"
        echo "Please Keep Colab Tab Open, Maximum Time 12h"
